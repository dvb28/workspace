    ...
    public partial class formHoaDon : Form
    {
        ...
        private void hoadonSearchBtn_Click(object sender, EventArgs e) {
            ...
            } else
            {
                ...
                {
                    MessageBox.Show($"Mã máy '{hoadonSearch.Text}' không tồn tại", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    hoadonSearch.Focus();
                }
            }
        }
        ...
        private void xuatHoaDon_Click(object sender, EventArgs e) {
            if(MessageBox.Show($"Bạn muốn xuất hóa đơn có mã {hoadonTable.CurrentRow.Cells["MaHD"].Value.ToString()}", "Confirm", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                ...
                Document crytalReport = new Document($@"..\..\CrystalReport\ReportSample\NHOM1.doc");
                ...
                crytalReport.MailMerge.Execute(new[] { "TenMay" }, new[] { hoadonTable.CurrentRow.Cells["TenMay"].Value.ToString() });
