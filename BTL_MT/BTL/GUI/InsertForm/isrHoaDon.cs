...
namespace BTL.GUI.InsertForm {
    public partial class isrHoaDon : Form {
        ...
        public void loadHangHoa() {
            try
            {
                SqlDataAdapter da = new SqlDataAdapter("SELECT MaMay, TenMay FROM MayTinh", clsHangHoa_BUS.strConnection);
                DataTable dt = new DataTable();
                da.Fill(dt);
                for (int j = 0; j < dt.Rows.Count; j++)
                {
                    string text = dt.Rows[j]["MaMay"].ToString() + " : " + dt.Rows[j]["TenMay"].ToString();
                    this.txtMaHang.Items.Add(text);
                    txtMaHang.ValueMember = "MaMay";
                }
            }
            catch (Exception ex)
            {
        ...
        private void hanghoaBtnUpdate_Click(object sender, EventArgs e) {
            ...
            } else
            {
                if (MessageBox.Show("Bạn có chắc chắn muốn thêm?!", "Add Document", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    clsHangHoa_BUS.updateHoaDon(dvg, txtMaHD.Text, txtMaHang.Text, txtMaNV.Text, txtMaKH.Text, txtNgayHD.Text);
